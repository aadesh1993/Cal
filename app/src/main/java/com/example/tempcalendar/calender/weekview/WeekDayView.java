...
public class WeekDayView extends View {
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            mDistanceX = distanceX * mXScrollingSpeed*2;
...
//            ViewCompat.postInvalidateOnAnimation(WeekNoHeaderView.this);
    ...
    private void init() {
        ...
        mTimeTextPaint.setColor(Color.parseColor("#737070"));
        Rect rect = new Rect();
        ...
        mEventTextPaint.setColor(Color.parseColor("#737070"));
     //   Typeface typeface = Typeface.create(Typeface.createFromAsset(mContext.getAssets(), mContext.getResources().getString(R.string.light)), Typeface.BOLD);
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    RectF eventRectF = new RectF(left, top, right-2, bottom);
                    if (bottom > 0 + mHeaderMarginBottom + mTimeTextHeight / 2 && left < right &&
                        ...
                        mEventBackgroundPaint.setColor(Color.parseColor("#ffe3e1"));
                        //canvas.drawRect(mEventRects.get(i).rectF, mEventBackgroundPaint);
                        canvas.drawRoundRect(mEventRects.get(i).rectF, 10, 10,  mEventBackgroundPaint);    //
                       // canvas.drawRect(r, paint);

                        // border
                        mEventBackgroundPaint.setStyle(Paint.Style.STROKE);
                        mEventBackgroundPaint.setColor(Color.parseColor("#ee4136"));
                        mEventBackgroundPaint.setStrokeWidth(3);
                        canvas.drawRoundRect(mEventRects.get(i).rectF, 10, 10,  mEventBackgroundPaint);
                      //  canvas.drawRect(mEventRects.get(i).rectF, mEventBackgroundPaint);
                        Log.e("Drawing events",""+mEventRects.get(i).event.getName());
